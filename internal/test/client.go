...
func NewTestClientBuilder() *ClientBuilder {
	return &ClientBuilder{}
}

// WithClient sets the client.Client to be used as a delegate for the test client.
func (b *ClientBuilder) WithClient(client client.Client) *ClientBuilder {
	b.delegatingClient = client
	return b
}

// RecordErrorForObjects records an error for a specific client.Client method and object keys.
func (b *ClientBuilder) RecordErrorForObjects(method ClientMethod, err *apierrors.StatusError, objectKeys ...client.ObjectKey) *ClientBuilder {
...
func (b *ClientBuilder) RecordErrorForObjectsMatchingLabels(method ClientMethod, namespace string, matchingLabels map[string]string, err *apierrors.StatusError) *ClientBuilder {
	// this method records error, so if nil error is passed then there is no need to create any error record.
	if err == nil {
		return b
	}
